...
def MBConv(dim_in, dim_out, *, downsample, expansion_rate = 4, shrinkage_rate = 0.25, dropout = 0.):
...
class OSAG(nn.Module):
    def __init__(self, channel_num=64, bias = True, block_num=1,**kwargs):
        super(OSAG, self).__init__()

        ffn_bias    = True  # kwargs.get("ffn_bias", False)
        window_size = 8     # kwargs.get("window_size", 0)
        pe          = True  # kwargs.get("pe", False)
...
class OmniSR(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx in range(len(self.residual_layer)):
            out = self.residual_layer[idx](out)
            if idx in [0, 2, 4]:
                out_feats.append(out)
