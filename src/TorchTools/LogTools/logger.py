...
class Logger(object):
    ...
    def __init__(self, name, exp_dir, opt, commend='',
        ...
        self.exp_dir = os.path.join('../experiments', exp_dir)
        self.log_dir = os.path.join(self.exp_dir, log_dir)
        # print(self.exp_dir, self.log_dir)
        # import pdb
        # pdb.set_trace()
    ...
    def save(self, name, model, optim, epoch, best_Ypsnr, best_epoch, dataparallel=1):
        """
        Torch save
        :param name:
        :param state_dict:
    ...
    def print_log(self, string, with_time=False, same_line=False):
        if with_time:
