...
import modules.pytorch_ssim as pytorch_ssim
...
## Test Dataset
parser.add_argument('--test_dataroot', type=str, default='../Dataset/DRealSR/Test_x4')
...
parser.add_argument('--pretrain', type=str, default='../Pretrained/IMDN_X4_IBSD.pth')
# parser.add_argument('--pretrain', type=str, default='')
parser.add_argument('--workers', type=int, default=8, help='number of threads to prepare data.')
# Logger
parser.add_argument('--result_dir', type=str, default='IMDN_IBSD', help='folder to sr results')
...
if 'RRDB-Fea' in opt.pretrain:
    opt.model = 'RRDB-Fea'
if 'IMDN' in opt.pretrain:
    opt.model = 'IMDN'
if 'RFDN' in opt.pretrain:
    opt.model = 'RFDN'
if 'ESRT' in opt.pretrain:
    opt.model = 'ESRT'
if 'ShuffleMix' in opt.pretrain:
    opt.model = 'ShuffleMix'
if 'OmniSR' in opt.pretrain:
    opt.model = 'OmniSR'


# Make Result Folder
result_root_dir = '../results/' + opt.model
exp_name = opt.result_dir
opt.result_dir = os.path.join(result_root_dir, opt.result_dir)
if not os.path.exists(opt.result_dir):
    os.makedirs(opt.result_dir)
if not os.path.exists(opt.result_dir+'/results'):
    os.makedirs(opt.result_dir+'/results')
...
if opt.model == 'SRResNet':
    import modules.RCAN.srresnet as srresnet
    generator = srresnet.make_model(opt)
    ...
    import modules.RCAN.edsr as edsr
    ...
    import modules.RCAN.rdn as rdn
    generator = rdn.make_model(opt)
elif opt.model == 'IMDN':
    import modules.RCAN.imdn as imdn
    generator = imdn.make_model(opt)
elif opt.model == 'RFDN':
    import modules.RCAN.rfdn as rfdn
    generator = rfdn.make_model(opt)
elif opt.model == 'ESRT':
    import modules.RCAN.esrt as esrt
    generator = esrt.make_model(opt)
elif opt.model == 'ShuffleMix':
    import modules.RCAN.shufflemix as shufflemix
    generator = shufflemix.make_model()
elif opt.model == 'OmniSR':
    import modules.RCAN.omni as omni
    generator = omni.make_model(opt)
elif opt.model == 'HGSR':
...
for batch, data in enumerate(test_loader):
    ...
    with torch.no_grad():
        ...
        for idx in range(iters):
            ...
            # sr_var = F.interpolate(hr_var, scale_factor=4, mode='bicubic')
            sr_var, _ = generator(hr_var)
    ...
    if opt.save_results:
        ...
        im.save(os.path.join(opt.result_dir+'/results', im_name))
    ...
    if opt.has_GT:
        psnr_single_rgb = psnr(sr_img, hr, peak=opt.rgb_range)
        ...
        with torch.no_grad():
            ...
            # LPIPS_single = 0
        print('%s PSNR_RGB: %.4f PSNR: %.4f SSIM: %.4f LPIPS: %.4f' % (im_name, psnr_single_rgb, psnr_single, ssim_single, LPIPS_single))
        log_f.write('image_name: ' + str(im_name) + '\t' + 'PSNR_RGB:' + str(psnr_single_rgb) + '\t' + 'PSNR:' + str(psnr_single) + '\t' + 'SSIM:' + str(ssim_single) + '\t' + 'LPIPS:' + str(LPIPS_single) + '\n')
        psnr_sum_rgb += psnr_single_rgb
...
if opt.has_GT:
    psnr_sum_rgb /= len(test_loader)
    psnr_sum /= len(test_loader)
    ssim_sum /= len(test_loader)
    LPIPS_sum /= len(test_loader)
    print('-----------\nAve: PSNR_RGB: %.4f PSNR: %.4f SSIM: %.4f LPIPS: %.4f\n-----------' % (psnr_sum_rgb, psnr_sum, ssim_sum, LPIPS_sum))
    log_f.write('\n-----------------------------------------------------------\n')
    

    log_f.write('\n PSNR_RGB:' + str(psnr_sum_rgb) + '\t' + 'PSNR: ' + str(psnr_sum) + '\t' + 'SSIM: ' + str(ssim_sum) + '\t' + 'LPIPS: ' + str(LPIPS_sum))
    log_f.write('\n \n --------------------------------------------------------------------- \n\n\n\n\n')
    sys.stdout.flush()
